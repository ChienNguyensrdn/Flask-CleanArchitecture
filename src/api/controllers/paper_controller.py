...
paper_service = PaperService(PaperRepository(session), PaperAuthorRepository(session))
author_service = PaperAuthorService(PaperAuthorRepository(session))
...
def submit_paper():
    ...
    try:
        ...
        # Get current user ID (from auth token/session)
        submitter_id = data.get('submitter_id', 1)  # TODO: Get from auth
